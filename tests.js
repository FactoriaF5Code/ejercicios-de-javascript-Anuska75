...
describe("Operaciones aritméticas", () => {
    ...
    it("Usando multiplicación", (b = 1.3) => {
        const a = 10;

        expect(a*b).to.equal(13);
    })
    it("Usando división", (b = 4) => {
        const a = 10;
...
describe("Strings", () => {
    it("Los strings representan cadenas de texto", () => {
        ...
        expect(phrase).to.equal("Cuenta con mi espada!")
    });
});
...
describe("Colecciones en JS: Array", () => {
    ...
    it("Tienen una longitud correspondiente que podemos conocer con la propiedad 'length'", () => {
        ...
        expect(lengthOfList).to.equal(4);
    })
...
describe("Operaciones iterables", () => {
    ...
    describe("Bonus: Utilizando map en una lista", () => {
        it("Duplicar cada número de la lista", () => {
    
            let numbers = [1, 2, 3, 4, 5];
    
            let doubledNumbers = numbers.map(numero => numero * 2);

            expect(doubledNumbers.every((num, index) => num === numbers[index] * 2)).to.equal(true);
        });
    });
